...
class ClientHandler:
    def __init__(self, conn, shared_list, book_number):
        ...
        self.conn.setblocking(False) # Non-I/O blocking的意思
    ...
    def read(self):
        try:
            ...
            if data:
                ...
                print(f"from Book {self.book_number}, added Node with batch data: \n{decoded_data}\n")
            else:
    ...
    def run(self):
        while True:
            events = self.selector.select(timeout=1)  # timeout set to 1 second
            if events:
                for key, _ in events:
                    callback = key.data
                    callback()
                
            if time.time() - self.last_received_time > 10:  # 5 seconds threshold
