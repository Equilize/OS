...
class RandMMU(MMU):
    def __init__(self, frames):
        ...
        self.debug = False
        self.list_page = [] #to store page number and if it is dirty page
        self.page_set = set()
    ...
    def page_access(self, page_number, rw_mode):
        ...
        r_after_w = False
        #if the page is not in the frame, then increment page fault by 1
        if page_number not in self.page_set:
            ...
            if self.frames == len(self.list_page):
                #if yes, then pop a random page in the list
                temp_replace_num = random.randint(0, self.frames - 1)
                replaced_page, dirty = self.list_page.pop(temp_replace_num)
